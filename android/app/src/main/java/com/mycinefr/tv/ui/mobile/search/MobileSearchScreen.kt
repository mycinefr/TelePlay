package com.mycinefr.tv.ui.mobile.search
...
import com.mycinefr.tv.ui.search.SearchViewModel
...
fun MobileSearchScreen(
    ...
    Column(
        ...
        Box(
            ...
            placeholder = { Text("Recherche de fichiers...", color = MobileTextSecondary) },
            ...
            textStyle = MaterialTheme.typography.bodyLarge.copy(color = MobileTextPrimary)
                    ...
                    if (uiState.query.isNotEmpty()) {
                        IconButton(onClick = { viewModel.onQueryChange("") }) {
                            Icon(Icons.Default.Close, "Nettoyer", tint = MobileTextSecondary)
                        }
                    }
                }
            }
        ...
        if (uiState.isSearching) {
            ...
            Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
             ...
             Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
                if (uiState.query.isNotEmpty()) {
                    Text("Aucun résultat pour \"${uiState.query}\"", color = MobileTextSecondary)
                } else {
                    Text("Saisissez votre recherche", color = MobileTextSecondary)
                }
            }
        } else {
...
@Composable
fun SearchFileCard(
    file: com.mycinefr.tv.data.model.FileItem,
    serverUrl: String,
    onClick: (Int) -> Unit,
