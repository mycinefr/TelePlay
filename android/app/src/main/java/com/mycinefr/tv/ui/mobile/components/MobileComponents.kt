package com.mycinefr.tv.ui.mobile.components
...
fun FileOptionsButton(
    ...
    Box {
        ...
        DropdownMenu(expanded = showMenu, onDismissRequest = { showMenu = false }) {
            DropdownMenuItem(
                text = { Text("Ouvrir dans un lecteur externe") },
                leadingIcon = { Icon(Icons.AutoMirrored.Filled.OpenInNew, null) },
                onClick = { onExternalPlayer(); showMenu = false }
            )
            onGoToFolder?.let {
                DropdownMenuItem(
                    text = { Text("Aller à l'emplacement du dossier") },
            ...
            HorizontalDivider()
            DropdownMenuItem(
                text = { Text(if (file.publicHash != null) "Copier le lien public" else "Générer le lien public") },
                leadingIcon = { Icon(Icons.Default.Share, null) },
                onClick = { onCopyPublic(); showMenu = false }
            )
            if (file.publicHash != null) {
                DropdownMenuItem(
                    text = { Text("Révoquer le lien public") },
                    leadingIcon = { Icon(Icons.Default.LinkOff, null) },
                    onClick = { onRevokePublic(); showMenu = false }
                )
            }
            DropdownMenuItem(
                text = { Text("Copier le lien de téléchargement") },
...
fun FolderOptionsButton(onDelete: () -> Unit, onMove: () -> Unit) {
    ...
    Box {
        ...
        DropdownMenu(expanded = showMenu, onDismissRequest = { showMenu = false }) {
            DropdownMenuItem(text = { Text("Déplacer") }, onClick = { onMove(); showMenu = false })
            DropdownMenuItem(text = { Text("Supprimer") }, onClick = { onDelete(); showMenu = false })
        }
    }
}
...
fun InputDialog(title: String, initialValue: String = "", onDismiss: () -> Unit, onConfirm: (String) -> Unit) {
    ...
    AlertDialog(
        ...
        confirmButton = { TextButton(onClick = { onConfirm(text) }) { Text("Confirmer") } },
    ...
    AlertDialog(
        ...
        text = {
            Column(modifier = Modifier.fillMaxWidth()) {
                Text("Sélectionnez le dossier cible :", style = MaterialTheme.typography.bodyMedium, modifier = Modifier.padding(bottom = 16.dp))
